...
##### Constants
#####
def SetOxideParameters(device, region, T, eps_ox=3.9):
    '''
...
def SetSiliconParameters(device, region, T=300, esp_si=11.1, n_i=1E10, mu_n=400, mu_p=200, tau_n=1E-5, tau_p=1E-5):
    ...
    set_parameter(device=device, region=region, name="Permittivity", value=esp_si * eps_0)
    set_parameter(device=device, region=region, name="ElectronCharge", value=q)
...
def CreateSiliconPotentialOnly(device, region):
    ...
    # kahan3 is a extended precision addition
    # require NetDoping
    # Create nodemodels
    for name, eq in (
...
def CreateSiliconDriftDiffusion(device, region, mu_n="mu_n", mu_p="mu_p"):
    """
    Sets up drift diffusion solutions for a single homogenious material like silicon
    :param device:
    :param region:
    :param mu_n:
    :param mu_p:
...
def CreatePE(device, region):
    ...
    :return:
    """
    # Kahan3 is the extended precision sum
...
def CreateSRH(device, region):
    """
    Creates modles and equations for Shockley Reed Hall recombination using a simple model
    Et = 1/2 Eg
    Capture cross sections and defect density becomes lifetime terms
...
def CreateECE(device, region, mu_n):
    """
    Creates the electron continuity equation
...
def CreateSiliconSiliconInterface(device, interface, electrons_name='Electrons', holes_name='Holes',
    ...
    interface_equation(device=device, interface=interface, name=ece_name, variable_name=electrons_name,
                       interface_model=ename, type="continuous")
    hname = CreateContinuousInterfaceModel(device, interface, holes_name)
    interface_equation(device=device, interface=interface, name=hce_name, variable_name=holes_name,
