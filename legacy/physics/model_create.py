...
def CreateContactEdgeModel(device, contact, model, expression):
    ...
    logger.debug(("CONTACTEDGEMODEL {d} {c} {m} \"{re}\"".format(d=device, c=contact, m=model, re=result)))
...
def CreateInterfaceModel(device, interface, model, expression):
    ...
    logger.debug(("INTERFACEMODEL {d} {i} {m} \"{re}\"".format(d=device, i=interface, m=model, re=result)))
...
def EnsureEdgeFromNodeModelExists(device, region, nodemodel):
    ...
    if not emtest:
        logger.info("INFO: Creating ${0}@n0 and ${0}@n1".format(nodemodel))
...
def CreateElementModelDerivative2d(device, region, model_name, expression, *args):
    ...
    for i in args:
        for j in ("@en0", "@en1", "@en2"):
            CreateElementModel2d(device, region, "{0}:{1}{2}".format(model_name, i, j),
